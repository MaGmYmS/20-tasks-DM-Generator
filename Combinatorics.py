...
class CombinatoricsTaskGenerator:
    ...
    def file_1_task_3_generate_binomial_variance_tasks(number_of_tasks):
        ...
        Человек планирует отдохнуть на этом курорте 10 дней. Случайная величина X – число солнечных дней за период
        отдыха человека. Дисперсия этой случайной величины равна:
    ...
    def file_2_task_1_generate_sample_mode_tasks(self, number_of_tasks):
        ...
        for _ in range(number_of_tasks):
            ...
            # Находим три числа, которые встречаются максимально часто после моды
            freq_dict = {num: sample.count(num) for num in set(sample) if num != mode}
            sorted_freq = sorted(freq_dict.items(), key=lambda x: x[1], reverse=True)
            wrong_answers = [item[0] for item in sorted_freq[:3]]
            ...
            result_tasks_massive.append((task_text, [mode], wrong_answers))
    ...
    def file_2_task_3_generate_sample_variance_tasks(number_of_tasks):
        ...
        for _ in range(number_of_tasks):
            ...
            mean = sum(sample) / n
            variance = round(sum((x - mean) ** 2 for x in sample) / n, 2)
    ...
    def file_2_task_5_generate_sample_variance_tasks(number_of_tasks):
        """
        Генерирует задачи по определению выборочной дисперсии по исправленному выборочному среднему квадратическому
        отклонению.
    ...
    def file_3_task_4(number_of_tasks):
        """
        Если наблюдаемое значение критерия (критической статистики) гипотезы попадает в область правдоподобных
        значений, то это означает что(правильный ответ – b)
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Если наблюдаемое значение критерия (критической статистики) гипотезы попадает в область "
                "правдоподобных значений, то это означает что")
    ...
    def file_3_task_9(number_of_tasks):
        """
        Если наблюдаемое значение критерия (критической статистики) гипотезы попадает в критическую область,
        то это означает что(правильный ответ –b)
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Если наблюдаемое значение критерия (критической статистики) гипотезы попадает в критическую "
                "область, то это означает что")
            correct_answer = "нулевая гипотеза отвергается"
            incorrect_answers = [
    ...
    def file_3_task_10(number_of_tasks):
        """
        Нулевая гипотеза – размер изделия совпадает с номинальным размером, наблюдаемое значение критерия попало
        в правую одностороннюю критическую область. В пользу какой гипотезы следует сделать выбор? (правильный ответ –d)
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Нулевая гипотеза – размер изделия совпадает с номинальным размером, наблюдаемое значение критерия "
                "попало в правую одностороннюю критическую область. В пользу какой гипотезы следует сделать выбор?")
    ...
    def file_3_task_11(number_of_tasks):
        """
        Нулевая гипотеза – средний рост взрослого мужчины, проживающего в городе N1, равен среднему росту взрослого
        мужчины, проживающего в городе N2, наблюдаемое значение критерия попало в левую одностороннюю критическую
        область. В пользу какой гипотезы следует сделать выбор? (правильный ответ – a)
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Нулевая гипотеза – средний рост взрослого мужчины, проживающего в городе N1, равен среднему росту "
                "взрослого мужчины, проживающего в городе N2, наблюдаемое значение критерия попало в левую "
                "одностороннюю критическую область. В пользу какой гипотезы следует сделать выбор?")
            correct_answer = ("средний рост взрослого мужчины из города N1 меньше, чем средний рост взрослого "
                              "мужчины из города N2")
            incorrect_answers = [
                "средний рост взрослого мужчины из города N1 больше, чем средний рост взрослого мужчины из города N2",
                "средний рост взрослого мужчины из города N1 отличается от среднего "
                "роста взрослого мужчины из города N2",
    ...
    def file_3_task_14(number_of_tasks):
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Вероятность того, что оцениваемые параметр действительно находится в найденном доверительном "
                "интервале, называется")
    ...
    def file_3_task_15(number_of_tasks):
        """
        По выборке объема 24 найдены два доверительных интервала для математического ожидания нормально распределенного
        признака X генеральной совокупности: 10<a<14 и 9<a<15. Верным является утверждение(правильный ответ – a)
...
есть центр ряда и прибавка к чему то,(12) правильно где более узкий интервал
        ...
        for i in range(number_of_tasks):
            ...
            while vtoroe == pervoe:
                vtoroe = random.randint(1, objee_chislo - 1)
            task_text = (
                f"По выборке объема {objee_chislo * 2} найдены два доверительных интервала для математического "
                f"ожидания нормально распределенного признака X генеральной совокупности: "
                f"{round(objee_chislo - pervoe, 2)}<a<{round(objee_chislo + pervoe, 2)} и "
                f"{round(objee_chislo - vtoroe, 2)}<a<{round(objee_chislo + vtoroe, 2)}. Верным является утверждение")

            if pervoe < vtoroe:
                correct_answer = ("надежность первой интервальной оценки меньше, чем надежность "
                                  "второй интервальной оценки")
                incorrect_answers = [
                    "надежность первой интервальной оценки больше, чем надежность второй интервальной оценки",
                    "надежность первой и второй интервальной оценок несравнимы",
                    "точность первой и второй интервальной оценок несравнимы"]
            else:
                correct_answer = ("надежность первой интервальной оценки больше, чем надежность второй "
                                  "интервальной оценки")
                incorrect_answers = [
                    "надежность первой интервальной оценки меньше, чем надежность второй интервальной оценки",
                    "надежность первой и второй интервальной оценок несравнимы",
    ...
    def file_3_task_16(number_of_tasks):
        """
        По двум разным выборкам с надежностью 0,95 найдены доверительные интервалы для среднего квадратического
        отклонения нормально распределенного признака X генеральной совокупности: 1,26<σ<2,74 и 1,44<σ<2.56.
        Выборочные дисперсии, найденные по этим выборкам, приблизительно равны. Верным
        является утверждение(правильный ответ – b)
        ...
        нужно чтобы один был в другом, ищем больший объем
        ...
        for i in range(number_of_tasks):
            ...
            while pervoe == vtoroe:
                vtoroe = round(random.uniform(0.5, objee_chislo), 2)
            task_text = (
                f"По двум разным выборкам надежностью 0,95 найдены доверительные интервалы для среднего "
                f"квадратического отклонения нормально распределенного признака X генеральной совокупности: "
                f"{round(objee_chislo - pervoe, 2)}<σ<{round(objee_chislo + pervoe, 2)} и "
                f"{round(objee_chislo - vtoroe, 2)}<σ<{round(objee_chislo + vtoroe, 2)}. "
    ...
    def file_4_task_1(number_of_tasks):
        ...
        Верным является утверждение (правильный ответ – с)
            a.	выборочный коэффициент корреляции является интервальной статистической оценкой генерального
            коэффициента корреляции
            b.	выборочный коэффициент корреляции равен генеральному коэффициенту корреляции
            c.	выборочный коэффициент корреляции является точечной статистической оценкой генерального
            коэффициента корреляции
            d.	выборочный коэффициент корреляции равен генеральному коэффициенту корреляции при условии
            совместного нормального распределения исследуемых случайных величин
        ...
        for i in range(number_of_tasks):
            ...
            correct_answer = ("выборочный коэффициент корреляции является точечной статистической оценкой "
                              "генерального коэффициента корреляции")
            incorrect_answers = [
                "выборочный коэффициент корреляции является интервальной статистической оценкой "
                "генерального коэффициента корреляции",
                "выборочный коэффициент корреляции равен генеральному коэффициенту корреляции",
                "выборочный коэффициент корреляции равен генеральному коэффициенту корреляции при "
                "условии совместного нормального распределения исследуемых случайных величин"]
            result = (task_text, [correct_answer], incorrect_answers)
            list_tasks.append(result)
    ...
    def file_4_task_2(number_of_tasks):
        ...
        Верным является утверждение (правильный ответ – a)
            a.	если коэффициент корреляции равен -1, то исследуемые случайные величины связаны линейной
            функциональной зависимостью
            b.	если коэффициент корреляции равен -1, то исследуемые случайные величины связаны функциональной
            зависимостью, причем не обязательно линейной
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Верным является утверждение")
            correct_answer = ("если коэффициент корреляции равен -1, то исследуемые случайные величины связаны "
                              "линейной функциональной зависимостью")
            incorrect_answers = [
                "если коэффициент корреляции равен -1, то исследуемые случайные величины связаны функциональной "
                "зависимостью, причем не обязательно линейной",
    ...
    def file_4_task_3(number_of_tasks):
        """
        Если в результате проверки гипотезы о равенстве нулю генерального коэффициента корреляции p-value=0,03,
        то (правильный ответ –d)
...
ответ меньше pvalue , pvalue до 1 десятой
    ...
    def file_4_task_6(number_of_tasks):
        """
        При исследовании зависимости между случайными величинамиX и Yполучено значение выборочного коэффициента
        корреляции 0,6, а при исследовании зависимости между случайными величинами X и Z получено значение
        выборочного коэффициента корреляции (-0,8). Все случайные величины имеют нормальное распределение.
        Верным является утверждение (правильный ответ – c)
...
сравниваем по модулю, то что бельше то более тесная связь (-1;1)
        ...
        for i in range(number_of_tasks):
            ...
            if pervoe < vtoroe:
                ...
                incorrect_answers = [
                    f"выборочный коэффициент корреляции между случайными величинами Y и Z равен "
                    f"{round(pervoe * vtoroe, 2)}",
    ...
    def file_4_task_7(number_of_tasks):
        """
       Верным является утверждение (правильный ответ – b)
            a.	если корреляционное отношение равно 1, то исследуемые случайные величины связаны линейной
            функциональной зависимостью
            b.	если корреляционное отношение равно 1, то исследуемые случайные величины связаны функциональной
            зависимостью, причем не обязательно линейной
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Верным является утверждение")
            correct_answer = ("если корреляционное отношение равно 1, то исследуемые случайные величины "
                              "связаны функциональной зависимостью, причем не обязательно линейной")
            incorrect_answers = [
                "если корреляционное отношение равно 1, то исследуемые случайные величины связаны линейной "
                "функциональной зависимостью",
...
a.	если корреляционное отношение равно 0, то исследуемые случайные величины могут быть как зависимыми,
так и независимыми
...
d.	если корреляционное отношение равно 0, то исследуемые случайные величины независимы
        ...
        for i in range(number_of_tasks):
            ...
            incorrect_answers = [
                ...
                "если корреляционное отношение равно 0, то исследуемые случайные величины могут быть как "
...
d.	правосторонняя или левосторонняя критическая область в зависимости от вида альтернативной гипотезы
    ...
    def file_4_task_12(number_of_tasks):
       ...
       (правильный ответ – b)
        ...
        for i in range(number_of_tasks):
            task_text = (
                f"При исследовании зависимости между случайными величинами X и Y получено значение выборочного "
                f"коэффициента корреляции {koef} и значение выборочного корреляционного отношения {otn}. "
                f"Верным является утверждение")
            ...
            incorrect_answers = [
                ...
                "такого не может быть, потому что если выборочный коэффициент корреляции положительный, "
                "то он должен быть равен"]
            result = (task_text, [correct_answer], incorrect_answers)
            list_tasks.append(result)
    ...
    def file_4_task_14(number_of_tasks):
       ...
       Верным является утверждение (правильный ответ – b)
            ...
            c.	передвычислением выборочного коэффициента корреляции необходимо проверить гипотезу о
            равенстве генеральных средних исследуемых случайных величин
            d.	перед вычислением выборочного корреляционного отношения необходимо проверить гипотезу о
            равенстве генеральных дисперсий исследуемых случайных величин
        ...
        for i in range(number_of_tasks):
            task_text = (
                "Верным является утверждение")
            correct_answer = ("при вычислении выборочного корреляционного отношения необходимо "
                              "группировать данные в интервалы")
            incorrect_answers = [
                "при вычислении выборочного коэффициента корреляции необходимо группировать данные в интервалы",
                "перед вычислением выборочного коэффициента корреляции необходимо проверить гипотезу "
                "о равенстве генеральных средних исследуемых случайных величин",
                "перед вычислением выборочного корреляционного отношения необходимо проверить гипотезу "
                "о равенстве генеральных дисперсий исследуемых случайных величин"]
            result = (task_text, [correct_answer], incorrect_answers)
            list_tasks.append(result)
    ...
    def file_specific_task_star(number_of_tasks):
        """
       Случайная величина принимает значения 1 и 2 с вероятностями 0,6 и 0,4 соответственно.
       Дисперсия этой случайной величины равна (правильный ответ – b)
            a.	1,4
            b.	0,24
            c.	0,76
            d.	2,2
        ...
        for i in range(number_of_tasks):
            ...
            ver_2 = 1 - ver_1
            task_text = (
                f"Случайная величина принимает значения 1 и 2 с вероятностями {ver_1} и {ver_2} соответственно. "
                f"Дисперсия этой случайной величины равна")
    ...
    def logic_1_task_combinatorics_six_different_gender_pairs(number_of_tasks):
        ...
        for _ in range(number_of_tasks):
            ...
            task_text = (f"На школьном вечере присутствуют {number_of_girls} девушек и {number_of_boys} юношей. "
                         f"Сколькими способами можно выбрать из них разнополых пар в количестве: {number_pairs} "
                         f"для танца?")
