...
class CombinatoricsTaskGenerator:
    ...
    def file_3_task_3(number_of_tasks):
        ...
        for i in range(number_of_tasks):
            task_text = ("Уровень значимости критерия - это")
            correct_answer = "вероятность отвергнуть гипотезу Н0, в то время как она верна"
            incorrect_answers = [
...
d.	правосторонняя или левосторонняя критическая область в зависимости от вида альтернативной гипотезы
    ...
    def file_4_task_10(number_of_tasks):
       ...
       Верным является утверждение (ρ – корреляционное отношение) (правильный ответ – c)
        ...
        for i in range(number_of_tasks):
            ...
            correct_answer = "\rho_{XY} \neq \rho_{YX} \\"
            incorrect_answers = [
                "\rho_{XY} = \rho_{YX} \\",
                "\rho_{XY} = \frac{1}{\rho_{YX}} \\",
                "\rho_{XY} = -\rho_{YX}"]
            result = (task_text, correct_answer, incorrect_answers)
            list_tasks.append(result)
    ...
    def file_4_task_11(number_of_tasks):
       ...
       Верным является утверждение (r – коэффициент корреляции) (правильный ответ – a)
        ...
        for i in range(number_of_tasks):
            ...
            correct_answer = "r_{XY} = r_{YX} \\"
            incorrect_answers = [
                "\rho_{XY} = \frac{1}{r_{YX}} \\",
                "\rho_{XY} \neq r_{YX} \\",
                "\rho_{XY} = -r_{YX}"]
            result = (task_text, correct_answer, incorrect_answers)
            list_tasks.append(result)
    ...
    def file_4_task_12(number_of_tasks):
        """
       При исследовании зависимости между случайными величинами X и Y получено значение выборочного коэффициента корреляции 0,2 и значение выборочного корреляционного отношения 0,9. Верным является утверждение (правильный ответ – b)
            ...
            d.	такого не может быть, потому что если выборочный коэффициент корреляции положительный, то он должен быть равен

выборочному корреляционному отношению корреляционного отношения 0,9(0.75 до 1)
выборочного коэффициента корреляции 0,2 (0 до 0.3)
    ...
    def file_4_task_13(number_of_tasks):
        """
       Если в результате проверки гипотезы о равенстве нулю генерального корреляционного отношенияp-value=0,02, то (правильный ответ –c)
            a.	эта гипотеза отвергается на любом уровне значимости
...
как в предыдущем но наоборот
    ...
    def file_1_task_3_generate_binomial_variance_tasks(number_of_tasks):
        ...
        for _ in range(number_of_tasks):
            ...
            # Составляем текст задачи
            task_text = (
                f"В курортном городе в летний период {p * 100}% дней по статистике являются солнечными. "
